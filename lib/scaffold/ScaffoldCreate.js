          ...
          ScaffoldUtils.dirExists( path.normalize(inputFolder + '/' + category), function(exists){
            if (!exists){
              ScaffoldBuilder.buildInside(category, path.normalize(inputFolder), false, function(){
                ...
                var infoJSONPath = path.normalize(inputFolder + '/' + category + '/info.json');
                var fd = fs.openSync(infoJSONPath, 'w');
                var data = fs.readFileSync('lib/data/category_info.json').toString();
