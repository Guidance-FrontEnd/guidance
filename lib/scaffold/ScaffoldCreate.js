      ...
      if (!exists){
        ...
        if (category){
          ScaffoldUtils.dirExists( path.normalize(inputFolder + '/' + category), function(exists){
            if (!exists){
              ScaffoldBuilder.buildInside(category, path.normalize(inputFolder), false, function(){
                var infoJSONFrom = path.normalize('lib/data/category_info.json');
                var infoJSONTo = path.normalize(inputFolder + '/' + category + '/info.json');
                ScaffoldUtils.copyFile(infoJSONFrom, infoJSONTo, function(){
